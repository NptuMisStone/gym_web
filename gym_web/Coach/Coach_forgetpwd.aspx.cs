...
public partial class Coach_Coach_forgetpwd : System.Web.UI.Page
{
    ...
    protected void btn_ok_Click(object sender, EventArgs e)
    {
        ...
        if (tb_pwd1.Text == verificationCode)
        {
            ...
            {
                // 驗證碼過期的處理逻辑
                ScriptManager.RegisterStartupScript(Page, typeof(Page), "ALERT", "alert('驗證碼已過期，請重新獲取');window.location='Coach_forgetpwd.aspx?A_Sno=1';", true);
            }
        }
        else
        {
            ScriptManager.RegisterStartupScript(Page, typeof(Page), "ALERT", "alert('驗證碼錯誤');window.location='Coach_forgetpwd.aspx?A_Sno=1';", true);
        }
    }
    protected void Button3_Click(object sender, EventArgs e)
    {
        ...
        using (SqlConnection sqlcn = new SqlConnection(connectionString))
        {
            ...
            string sql = "update 健身教練資料 set 健身教練密碼=@Coach_password where 健身教練帳號=@Coach_account";

            using (SqlCommand command = new SqlCommand(sql, sqlcn))
            {
                command.Parameters.AddWithValue("@Coach_password", tb_new_pwd.Text);
                command.Parameters.AddWithValue("@Coach_account", tb_acc.Text);
                ...
                {
                    // 更新成功的处理逻辑
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "ALERT", "alert('更改成功，請重新登入！');window.location='Coach_login.aspx?A_Sno=1';", true);
                }
                else
                {
                    // 更新失败的处理逻辑
